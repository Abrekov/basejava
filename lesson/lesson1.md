...
### Настройка проекта
- Создайте на GitHub репозиторий с названием `ru.webapp.basejava`
...
- Создайте локальную копию проекта: `git clone https://github.com/JavaOps/ru.webapp.basejava.git`
- Перейдите в каталог проекта: `cd ru.webapp.basejava`
...
- Создайте в IntelliJ IDEA новый проект, выбрав каталог `ru.webapp.basejava`, который вы клонировали ранее к себе на компьютер:
...
- Реализуйте класс `ru.webapp.basejava.storage.ArrayStorage`, организовав хранение резюме на основе массива с методами `save, get, delete, size, clear, getAll`
- Храните все резюме в начале `ru.webapp.basejava.storage` (без дырок в виде `null`), чтобы не перебирать каждый раз все 10000 элементов
```
Схема хранения резюме в массиве ru.webapp.basejava.storage (в элементах от 0 до size-1 отсутствуют null):

r1, r2, r3,..., rn, null, null,..., null
<----- size ----->
<------- ru.webapp.basejava.storage.length (10000) ------->
```
- Протестируйте вашу реализацию с помощью классов `ru.webapp.basejava.MainArray.main()` и `ru.webapp.basejava.MainTestArrayStorage.main()`
...
6. `ru.webapp.basejava.model.Resume r` — давай переменным осмысленные имена, например `resume`. `r` допустимо в коротких циклах и лямбдах.
